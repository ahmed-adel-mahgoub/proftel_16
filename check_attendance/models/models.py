...
class checkAattendance(models.Model):
    ...
    full_approval = fields.Boolean(compute='full_approval_action', search=True,
                                   store=True)
    ...
    manager_approval_check_out = fields.Integer(default=0, readonly=1,
                                                store=True)
    ...
    @api.depends('hr_approval_check_in', 'hr_approval_check_out',
                 'manager_approval_check_in', 'manager_approval_check_out')
    ...
    def _onchange_checkout_date(self):
        if self.check_out:
            ...
            if attendance_records:
                self.check_in = attendance_records[
                    0].check_in  # You can choose how to handle multiple records
                self.update = 1
